...
Assert-Contains $html "Escolha uma plataforma para continuar" "Missing required portal message"
Assert-Contains $html "href=`"/orcamentos/`"" "Missing orcamentos link"
Assert-Contains $html "<strong>Orçamentos</strong>" "Missing orcamentos link label"
...
Assert-Contains $html "<strong>Copiloto Documental</strong>" "Missing Documental link label"
Assert-Contains $html "<strong>Relatórios</strong>" "Missing relatorios link label"
Assert-Contains $html 'href="https://ia.solutiongroup.com.br/erosao"' "Erosao link must use the production portal URL"
Assert-Contains $html "<strong>APP de relatório de erosão</strong>" "Missing erosao link label"
Assert-Contains $html 'href="/gestao-ia"' "Missing central AI cost management link"
Assert-Contains $html "Administração e acessos" "Missing portal administration label"
Assert-Contains $html "Gerencie pessoas, permissões por plataforma" "Missing access management description"
Assert-NotContains $html "vercel.app" "Portal must not link to Vercel"

Assert-Contains $styles "--module-accent: var(--blue);" "Orcamentos card must use the institutional accent color"
Assert-Contains $styles ".module-link[href=`"/relatorios/`"] {`n  --module-accent: #c2414a;`n}" "Relatorios card must use a red accent color"
Assert-Contains $styles ".module-link[href=`"/documental`"] {`n  --module-accent: #2563a6;`n}" "Documental card must use a blue accent color"
Assert-Contains $styles ".module-link[href=`"https://ia.solutiongroup.com.br/erosao`"] {`n  --module-accent: #c77a00;`n}" "Erosao report card must use a yellow accent color"
Assert-Contains $styles "border-left: 4px solid var(--green);" "Administration entry must use the institutional green"
